...
static size_t pes_packet_read(struct ps_demuxer_t *ps, const uint8_t* data, size_t bytes)
{
        ...
        i += pes_packet_length) 
    {
        ...
        {
            std::cout << "i + 6 + pes_packet_length" << i << std::endl;
            return i;
        }
        ...
        default:
            ...
            pes_packet(&pes->pkt, pes, data + i + j, pes_packet_length - j, ps_demuxer_onpes, ps);
        }
    }
...
size_t ps_demuxer_input(struct ps_demuxer_t* ps, const uint8_t* data, size_t bytes)//bytes = 12708
{
	size_t i, n;
